            ...
            while ($check=$result->fetch_assoc()) {
                ...
                if(!$b){
                    foreach ($attendance as $key => $value) {
                        if($value=="present"){
                            $query="insert into class0(date,subject,r_no,attendance,value) values('$date','$subject',$key,'present',1)";
                            $insertResult=$link->query($query);
                        }
                        else{
                            $query="insert into class0(date,subject,r_no,attendance,value) values('$date','$subject',$key,'absent',0)";
                            $insertResult=$link->query($query);   
                        }
                    }
